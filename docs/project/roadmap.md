...
- **Sprint 4 — Telas de UI/UX:** `1B.2 (perfil)` → `1B.3 (salvos)` → `1B.5 (ajuda)` → `1B.4 (configurações)` → `1B.1 (boas-vindas grupo)` já feitos. Falta: `1B.9 (eventos, depois de 1.4)` → `1B.6 (notificações)`.
