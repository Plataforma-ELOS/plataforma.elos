...
- **Fora de escopo:** paginação de `/profissionais` — mesmo padrão pode ser aplicado depois, mas fica pendente; o volume de seed atual não justifica ainda. **Atualização 2026-07-28:** a causa raiz que bloqueava reabrir esse escopo (ver `E8` abaixo) já foi corrigida; a paginação de `/profissionais` propriamente dita continua pendente para uma etapa seguinte.

#### ✅ Critério de aceite
- [x] Feed de posts pagina (`.range()` no Supabase); busca/ordenação por curtidas continuam funcionando sobre os posts já carregados.
- [ ] `/profissionais` — pendente (ver nota acima).
...
- **E8 — implementado em 2026-07-28 para `/acervo-digital`:** coluna `search_vector` (`tsvector`, `generated always as`) + índice GIN em `library_items` (migration `add_library_items_search_vector`), combinando `title`+`author_name`+`tags` com `to_tsvector('portuguese', ...)`. **Achado técnico:** `array_to_string` (usado para juntar o array `tags` num texto) é `STABLE`, não `IMMUTABLE` — Postgres rejeita isso em coluna gerada (`generation expression is not immutable`). Corrigido com uma função wrapper `immutable_array_to_string` declarada `immutable` explicitamente (mesma função, só a garantia de pureza é assumida pelo autor — caminho documentado para esse caso específico de full-text search). Nova Server Action `buscarItensAcervo(query)` (`src/app/actions/library.ts`) usa `.textSearch('search_vector', query, { type: 'websearch', config: 'portuguese' })`, reaproveita `mapLibraryRow` e replica o merge de favoritos já existente em `acervo-digital/page.tsx`. `acervo-digital/client-page.tsx`: busca de texto desacoplada do `useSearch` (que ficou só com filtro de tipo + ordenação) — debounce de 300ms chama a Server Action e faz upsert dos resultados em `libraryItems`, preservando o toggle de favorito otimista.
  - **Fora de escopo, registrado como pendência (na época):** full-text em `/profissionais` — os chips de especialidade usavam um prefixo truncado (`'Fonoaudiólog'`) para casar com `.includes()` em JS contra variações de gênero; full-text do Postgres casa por lexema inteiro, não substring, e trocar isso sem cuidado quebraria essa UX.
  - **Causa raiz corrigida em 2026-07-28:** investigando esse truque, descobri que `/cadastro-profissional` (`inscreverProfissional`, `src/app/actions/professional-signup.ts`) nunca gravava `specialty` em nenhum dos dois inserts — só as linhas seed/demo tinham o campo preenchido; qualquer cadastro real ficava com `specialty = null`. Corrigido: `src/lib/data/specialties.ts` (novo) define a lista canônica (`Psicólogo(a)`, `Fonoaudiólogo(a)`, `Terapeuta Ocupacional`, `Neurologista ou Psiquiatra`, `Psicopedagogo(a)`, `Acompanhante Terapêutico(a)`), usada tanto pelo novo `<Select>` "Especialidade" em `cadastro-profissional/page.tsx` (com opção "Outro" revelando um campo livre) quanto pelos chips de `profissionais/client-page.tsx`; clínicas ganharam um campo de texto livre "Especialidade/Área de Atuação" (categoria diferente, descritiva). `inscreverProfissional` passou a gravar `specialty` nos dois inserts. Migration `canonicalize_professionals_specialty` ajustou os 5 valores de `specialty` das linhas demo para a lista canônica (clínicas não mudaram — specialty delas já era texto livre). Os chips deixaram de reaproveitar o estado de busca por texto (`query`) e agora controlam um filtro exato e desacoplado via `matchesFilter` do `useSearch` (`item.specialty === tag`), então busca por nome e filtro por especialidade funcionam de forma independente. **Paginação e full-text de `/profissionais` continuam pendentes** — a causa raiz que impedia reabrir esse escopo está resolvida, mas a etapa de paginação/busca em si é uma entrega separada.
- **Aceite:** [x] notificações chegam em tempo real · [x] busca global rápida server-side no Acervo Digital · [ ] full-text/paginação em `/profissionais` — causa raiz corrigida, paginação/busca ainda pendente.
